...
- **開発中は途中で「承認しますか」「実行していいですか」等をユーザーに求めないこと。** 方針に沿ってコマンド実行・修正・ビルドまで一気に最後まで進める（危険な破壊操作など、明確に避けるべき例外を除く）。
- 作業は最後まで一気に仕上げてください。作業終了時には GitHub にコミットし、プッシュし、最後にビルドまで行ってください。
- **コードの変更・修正のあとは必ずリリースビルドを行うこと。** `cargo build --release -p open-agents-gui` を実行し、成功（エラー 0）を確認する。デバッグビルドだけで終えない。
